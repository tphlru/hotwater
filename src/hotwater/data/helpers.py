...
class Timeframe:
    ...
    TIMEFRAME_PATTERN = re.compile(
        r"^(\d+(?:\.\d+)?)\s*(s|sec|second|seconds|min|minute|minutes|h|hour|hours|d|day|days|w|week|weeks|m|month|months|y|year|years)$",
        re.IGNORECASE,
    )
    ...
    UNIT_TO_SECONDS = {
        ...
        "m": 2592000,
        "month": 2592000,
        "months": 2592000,  # ~30 дней
        "y": 31536000,
        "year": 31536000,
        "years": 31536000,  # ~365 дней
    }
    ...
    def __init__(self, value: Union[str, int, float, timedelta_dt, "Timeframe"]):
        """
    ...
    def _parse_to_seconds(self, value: Union[str, int, float, timedelta_dt]) -> int:
        ...
        elif isinstance(value, timedelta_dt):
    ...
    def _normalize_string(self, value: Union[str, int, float, timedelta_dt]) -> str:
    ...
    @property
    def timedelta(self) -> timedelta_dt:
        """Возвращает таймфрейм как timedelta."""
        return timedelta_dt(seconds=self._seconds)
    ...
    def __add__(self, other: Union["Timeframe", timedelta_dt]) -> "Timeframe":
        if isinstance(other, Timeframe):
            return Timeframe(timedelta_dt(seconds=self._seconds + other._seconds))
        elif isinstance(other, timedelta_dt):
            return Timeframe(
                timedelta_dt(seconds=self._seconds + other.total_seconds())
            )
        else:
            raise TypeError(f"Нельзя сложить Timeframe с {type(other)}")

    def __sub__(self, other: Union["Timeframe", timedelta_dt]) -> "Timeframe":
        if isinstance(other, Timeframe):
            return Timeframe(
                timedelta_dt(seconds=max(0, self._seconds - other._seconds))
            )
        elif isinstance(other, timedelta_dt):
            return Timeframe(
                timedelta_dt(seconds=max(0, self._seconds - other.total_seconds()))
            )
        else:
